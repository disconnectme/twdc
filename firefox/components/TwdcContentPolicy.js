 ...
 * @constructor
 */
function TwdcContentPolicy() {
	//allows access of native methods by scripts in content area
	this.wrappedJSObject = this;  	
}

TwdcContentPolicy.prototype =
{
	classDescription: "This is an image blocker",
	contractID: "@disconnect.me/twdc/contentpolicy;1",
	classID: Components.ID("{d32a3c00-4ed3-11de-8a39-0800200c9a64}"),
		...
		if(reqOrig != null && reqOrig.host!="browser" && contLoc.host!="browser" && contLoc.host!="global" && contType!=6){
			...
			if( reqOrig.host !=contLoc.host && !this.isMatching(reqOrig.host, this.DOMAINS) && this.isMatching(contLoc.host, this.DOMAINS) && typeof aContext.ownerDocument != null){
				...
				try{
					...
					if(typeof aContext.ownerDocument.TwdcCount == "undefined"){
						aContext.ownerDocument.TwdcCount = 1;
					}
					else{
						aContext.ownerDocument.TwdcCount += 1;						
					}					
					
					var finalBlocking = aContext.ownerDocument.defaultView.content.localStorage.getItem('TwdcStatus');
...
if (XPCOMUtils.generateNSGetFactory)
	var NSGetFactory = XPCOMUtils.generateNSGetFactory([TwdcContentPolicy]);
else
	var NSGetModule = XPCOMUtils.generateNSGetModule([TwdcContentPolicy]);
